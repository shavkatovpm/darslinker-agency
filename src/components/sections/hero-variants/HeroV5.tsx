...
export function HeroV5() {
  ...
  return (
    <section ref={sectionRef} className="relative min-h-screen flex flex-col items-center justify-center overflow-hidden pb-[8vh]">
      ...
      {/* Notification cards — appear after 3 seconds in bottom-right */}
      <motion.div
        initial={{ opacity: 0, x: 40, y: 20 }}
        animate={showCards ? { opacity: 1, x: 0, y: 0 } : { opacity: 0, x: 40, y: 20 }}
        transition={{ duration: 0.6, ease: "easeOut" }}
        className="absolute bottom-24 right-4 sm:right-8 lg:right-12 z-10"
      >
        <NotificationStack started={showCards} />
      </motion.div>
