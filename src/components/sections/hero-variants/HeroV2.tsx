  ...
  return (
    <section ref={sectionRef} className="relative min-h-screen flex flex-col items-center justify-center overflow-hidden pb-[8vh]">
      ...
      <div ref={contentRef} className="relative mx-auto w-full max-w-7xl px-4 sm:px-6 lg:px-8 will-change-transform">
        {/* Mobile: cards on top, text below. Desktop: text left, cards right */}
        <div className="flex flex-col lg:flex-row lg:items-center lg:justify-between lg:gap-12">
          {/* Text content — left aligned */}
          <div className="order-2 lg:order-1 text-left max-w-2xl">
          ...
          {/* Notification cards — right side on desktop, top on mobile */}
          <motion.div
            initial={{ opacity: 0 }}
            animate={showRest ? { opacity: 1 } : { opacity: 0 }}
            transition={{ duration: 0.4 }}
